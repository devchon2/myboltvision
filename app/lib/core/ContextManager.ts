...
export class ContextManager {
  public contextVectorDB: VectorDB;
  public liveContextCache = map<Partial<Record<string, ContextCluster>>>();
  ...
  public checkLiveCache(input: string): ContextCluster | undefined {
    const cache = this.liveContextCache.get();
    return Object.values(cache).find(ctx => 
      ctx?.content?.toLowerCase() === input.toLowerCase()
    );
  }

  public purgeStaleCacheEntries(): void {
    const currentTime = Date.now();
    const cacheContent = this.liveContextCache.get();
    const filteredEntries = Object.entries(cacheContent)
      .filter(([_, entry]) => entry && currentTime - entry.timestamp < 3600000); // 1 heure
  ...
  private calculateComplexity(text: string): number {
    ...
    const technicalTerms = [
      'algorithme', 'intelligence artificielle', 'api', 'architecture', 'async',
      'function', 'component', 'database', 'framework', 'implementation',
      'interface', 'library', 'method', 'module', 'object', 'pattern', 'protocol',
      'reference', 'service', 'system', 'transformer', 'variable', 'vector'
    ];
  ...
  private assessInnovation(text: string): number {
    // Protection contre les entrées vides ou invalides
    if (!text || typeof text !== 'string') {
      return 0.1;
    }
    ...
    const innovationCategories = {
      ...
      emerging: {
        terms: [
          'intelligence artificielle', 'ia générative', 'llm', 'large language model',
          'blockchain', 'web3', 'métavers', 'réalité augmentée', 'réalité virtuelle',
          'quantique', 'edge computing', 'iot', 'internet des objets',
          'transformers', 'multimodal', 'zero-shot', 'few-shot'
        ],
        weight: 0.4
      },
      ...
      innovation: {
        terms: [
          'innovant', 'révolution', 'disruptif', 'nouveauté', 'créatif',
          'pionnier', 'avant-garde', 'percée', 'paradigme', 'breakthrough',
          'futuriste', 'transformateur', 'visionnaire', 'émergent', 'novateur'
        ],
        weight: 0.25
      },
      ...
      technical: {
        terms: [
          'architecture', 'algorithme', 'optimisation', 'framework', 'infrastructure',
          'scalable', 'distribuée', 'cloud', 'serverless', 'microservices',
          'vectorisation', 'parallelisation', 'concurrent', 'asynchrone'
        ],
        weight: 0.15
      },
      ...
      domains: {
        terms: [
          'santé', 'finance', 'éducation', 'transport', 'énergie',
          'durable', 'écologique', 'vert', 'social', 'humanitaire'
        ],
        weight: 0.1
      }
    };
    ...
    Object.entries(innovationCategories).forEach(([_, category]) => {
      ...
      const categoryMatches = category.terms.filter(term => 
