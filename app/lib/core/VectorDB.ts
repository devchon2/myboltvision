import type { ContextCluster, ContextShard } from '../../types/context';
...
export class VectorDB {
  ...
  constructor(config: VectorDBConfig = {}) {
    ...
    this.CONNECTION_TIMEOUT = config.connectionTimeout || 10000; // 10 secondes
  ...
  async updateVectors(results: any[]): Promise<void> {
    ...
    if (!this.index || !results || results.length === 0) return;
