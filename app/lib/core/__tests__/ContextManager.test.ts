...
vi.mock('../VectorDB', () => {
  return {
    VectorDB: vi.fn().mockImplementation(() => ({
      ...
      findRelevant: vi.fn().mockResolvedValue([/* données mock */]),
  ...
  describe('Méthodes de cache', () => {
    ...
    describe('Algorithmes de calcul', () => {
      test('devrait calculer correctement la complexité pour différentes entrées', () => {
        ...
        // Texte plus complexe
        const complexText = 'Le système d\'intelligence artificielle implémente des algorithmes sophistiqués pour analyser et interpréter des données sémantiques complexes avec une précision remarquable.';
      ...
      test('devrait calculer correctement le potentiel d\'innovation pour différentes entrées', () => {
        ...
        const innovativeText = 'Intelligence artificielle générative révolutionnant la créativité multimodale avec une architecture transformer disruptive';
