...
export class MarketAnalysisAgent implements Agent {
  ...
  private analyzeRequestType(input: string): string {
    ...
    if (/concurrence|concurrent|compétiteur|compétition/i.test(lowerInput)) return 'competitive-analysis';
    if (/tendance|trend|évolution|futur|prévision/i.test(lowerInput)) return 'market-trends';
    if (/opportunité|niches|segments|croissance|potentiel/i.test(lowerInput)) return 'opportunity-identification';
    if (/swot|force|faiblesse|menace|avantage/i.test(lowerInput)) return 'swot-analysis';
  ...
  private async analyzeCompetitors(context: ContextCluster): Promise<string> {
    ...
    return 'Analyse des Concurrents:\n' +
           '## Principaux acteurs\n\n' +
           '1. Entreprise A\n' +
           '   - Part de marché: 35%\n' +
