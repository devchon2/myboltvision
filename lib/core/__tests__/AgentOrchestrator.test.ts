...
describe('AgentOrchestrator', () => {
  ...
  test("devrait gérer les erreurs lors de l'exécution d'un agent inexistant", async () => {
    ...
    await expect(orchestrator.executeAgent('agent-inexistant', input)).rejects.toThrow(
      'Agent with ID agent-inexistant not found',
    );
  });
  ...
  test('devrait gérer les erreurs dans un workflow', async () => {
    ...
    await expect(orchestrator.executeWorkflow(mockWorkflow.id, 'Error input')).rejects.toThrow();
  });
