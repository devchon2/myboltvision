...
import { webcontainer as webcontainerPromise } from '../webcontainer';

const lookupSavedPassword = (url: string) => {
  const domain = url.split('/')[2];
  const gitCreds = localStorage.getItem(`git:${domain}`);
...
const saveGitAuth = (url: string, auth: GitAuth) => {
  const domain = url.split('/')[2];
  localStorage.setItem(`git:${domain}`, JSON.stringify(auth));
};

export function useGit() {
  const [ready, setReady] = useState<boolean>(false);
  const [webcontainer, setWebcontainer] = useState<WebContainer | null>(null);
  const [fs, setFs] = useState<PromiseFsClient | null>(null);
      ...
      try {
        await git.clone({
          ...
          onAuthFailure: (url, _auth) => {
            ...
            throw new Error(`Error Authenticating with ${url.split('/')[2]}`);
          },
...
const getFs = (
  ...
  promises: {
    ...
    rmdir: async (path: string, options: any) => {
      ...
      try {
        const result = await webcontainer.fs.rmdir(relativePath, { recursive: true, ...options });
        return result;
      } catch (error: any) {
        throw new Error(error.message as string);
      }
...
const pathUtils = {
  ...
  relative: (from: string, to: string) => {
    return require('path-browserify').relative(from, to);
  },
};
