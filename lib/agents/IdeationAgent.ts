...
export class IdeationAgent implements Agent {
  ...
  private analyzeRequestType(input: string): string {
    ...
    // Analyse sémantique
    if (/brainstorm|remue-meninges|generate ideas|idées|nouveau projet/i.test(lowerInput)) {
      return 'brainstorming';
    }

    if (/concept|detaillé|developp|application/i.test(lowerInput)) {
