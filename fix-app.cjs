...
const harmonizeRemixVersions = () => {
  ...
  try {
    ...
    if (packageJson.resolutions) {
      ['@remix-run/react', '@remix-run/dev', '@remix-run/cloudflare', '@remix-run/server-runtime', '@remix-run/node'].forEach(pkg => {
        if (packageJson.resolutions[pkg] !== targetVersion) {
          console.log(`   ✓ Mise à jour de ${pkg}: ${packageJson.resolutions[pkg] || 'non défini'} -> ${targetVersion}`);
